...
for (selected_quarter in c("2021_Q2", "2022_Q1", "2022_Q3", 
                           "2023_Q3", "2023_Q4" )) {
  ...
  filename_ <- paste0("data_oos/data1_", selected_quarter, ".RData")
  ...
  # lets calculate EMA55 and EMA80 for NQ
  data.group1$NQ_EMA55 <- EMA(na.locf(data.group1$NQ), 55)
  data.group1$NQ_EMA80 <- EMA(na.locf(data.group1$NQ), 80)
  
  # and SP
  data.group1$SP_EMA55 <- EMA(na.locf(data.group1$SP), 55)
  data.group1$SP_EMA80 <- EMA(na.locf(data.group1$SP), 80)
  
  
  # put missing value whenever the original price is missing
  data.group1$NQ_EMA55[is.na(data.group1$NQ)] <- NA
  data.group1$NQ_EMA80[is.na(data.group1$NQ)] <- NA
  data.group1$SP_EMA55[is.na(data.group1$SP)] <- NA
  data.group1$SP_EMA80[is.na(data.group1$SP)] <- NA
  ...
  data.group1$positionSP.mom <- ifelse(lag.xts(data.group1$SP_EMA55) >
                                         lag.xts(data.group1$SP_EMA80),
                                       1, -1)
  ...
  if(!exists("quarter_stats_oos.all.group1")) quarter_stats_oos.all.group1 <- quarter_stats_oos else
    quarter_stats_oos.all.group1 <- rbind(quarter_stats_oos.all.group1, quarter_stats_oos)
  
  # create a plot of gros and net pnl and save it to png file
  png(filename = paste0("pnl_group1_oos_", selected_quarter, ".png"),
  ...
  print( # when plotting in a loop you have to use print()
    plot(cbind(cumsum(data.group1.daily$pnl_gross.mom),
         ...
         multi.panel = FALSE,
         main = paste0("Gross and net PnL for asset group 1 (MOM) \n quarter ", selected_quarter), 
         col = c("#377EB8", "#E41A1C"),
         major.ticks = "weeks", 
         grid.ticks.on = "weeks",
         grid.ticks.lty = 3,
  ...
  rm(data.group1, my.endpoints, grossSR, netSR, av.daily.ntrades,
     grossPnL, netPnL, stat, quarter_stats_oos, data.group1.daily)
...
write.csv(quarter_stats_oos.all.group1, 
          "quarter_stats_oos.all.group1.csv",
          row.names = FALSE)